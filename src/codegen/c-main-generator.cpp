...
void CiMainGenerator::Gen_MainSource()
{
  ...
  fwriter.Append("#endif // %s", fdesc->gen.usemon_def.c_str());
  fwriter.Append("");
  fwriter.Append("// This weak function is for the case when you need to enable");
  ...
  fwriter.Append("}");
  fwriter.Append("");
  fwriter.Append("// This weak function is for the case when you want to build");
  fwriter.Append("// app with enabled optoin auto CSM, but don't yet have");
  fwriter.Append("// proper getframehash implementation.");
  fwriter.Append("");
  fwriter.Append("__attribute__((weak)) uint32_t __get_hash__(uint32_t a, uint32_t b, uint32_t c, uint32_t d)");
  fwriter.Append("{");
  fwriter.Append("  (void)a;");
  fwriter.Append("  (void)b;");
  fwriter.Append("  (void)c;");
  fwriter.Append("  (void)d;");
  fwriter.Append("  return 0u;");
  fwriter.Append("}");
  fwriter.Append("");
  fwriter.Append("// This weak function is for the case when you need to enable");
  fwriter.Append("// using diag monitors but Check_Receive_Timeout function in");
  fwriter.Append("// binutil is not accessed.");
  fwriter.Append("");
  fwriter.Append("__attribute__((weak)) void __alert_reception_timeout__(uint32_t msgid, uint32_t lastcyc)");
  fwriter.Append("{");
  fwriter.Append("  (void)msgid;");
  fwriter.Append("  (void)lastcyc;");
  fwriter.Append("}");
  fwriter.Append("");
  fwriter.Append("// This weak function is for the case when you need to enable");
  fwriter.Append("// using diag monitors but Transmit function in binutil is");
  fwriter.Append("// not accessed.");
  fwriter.Append("");
  fwriter.Append("__attribute((weak)) int __send_can_message__(uint32_t msgid, uint8_t ide, uint8_t *d, uint8_t len)");
  fwriter.Append("{");
  fwriter.Append("  (void)msgid;");
  fwriter.Append("  (void)ide;");
  fwriter.Append("  (void)d;");
  fwriter.Append("  (void)len;");
  fwriter.Append("  return 1;");
  fwriter.Append("}");
  fwriter.Append("");
  fwriter.Append();
...
void CiMainGenerator::Gen_DbcCodeConf()
{
  ...
  fwriter.Append("");
  fwriter.Append("// extern uint32_t __get__tick__();");
  ...
  fwriter.Append("");
  fwriter.Append("// extern uint32_t __get_hash__(uint32_t a, uint32_t b, uint32_t c, uint32_t d);");
  fwriter.Append("// #define GetFrameHash(a,b,c,d,e) __get_hash__(a,b,c,d,e)");
  fwriter.Append("");
  fwriter.Append("// To provide a way to make missing control correctly you");
  fwriter.Append("// have to define macro @AlertReceptionTimeout(msgid,lastcyc)");
  fwriter.Append("// which alerts that a can message has not been received for");
  fwriter.Append("// five times the cycle time.");
  fwriter.Append("// the function should have following fields:");
  fwriter.Append("//  - msgid: id of the timeout message");
  fwriter.Append("//  - lastcyc: last cycle the timeout message was received");
  fwriter.Append("");
  fwriter.Append("// extern void __alert_reception_timeout__(uint32_t msgid, uint32_t lastcyc);");
  ...
  fwriter.Append("// which sends can message and return 0 for sent successfuly,");
  fwriter.Append("// and any other int for sent failed.");
  fwriter.Append("// the function should have following fields:");
  fwriter.Append("//  - msgid: id of the message");
  fwriter.Append("//  - ide: extended id flag");
  fwriter.Append("//  - d: data pointer");
  fwriter.Append("//  - len: length of the data");
  fwriter.Append("");
  fwriter.Append("// extern int __send_can_message__(uint32_t msgid, uint8_t ide, uint8_t *d, uint8_t len);");
  fwriter.Append("// #define SendCanMessage(msgid,ide,d,len) __send_can_message__(msgid,ide,d,len)");
  fwriter.Append("");
